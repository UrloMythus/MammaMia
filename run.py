        ...
        if type == "tv":
            for channel in STREAM["channels"]:
                if channel["id"] == id:
                    ...
                    if id in okru:
                        ...
                        streams['streams'].append({'title':  f"{Icon}Server {i} " +  channel['title'] + " OKRU [IT]",'url': channel_url, 'languages': ['it'], "behaviorHints": {"notWebReady": True, "proxyHeaders": {"request": {"User-Agent": User_Agent}}}})
                    if id in extra_sources:
                        list_sources = extra_sources[id]
                        for item in list_sources:
                            i = i+1
                            if "iran" in item:
                                streams['streams'].append({'title':f"{Icon}Server {i} " + channel['title'] + " [IT]",'url': item, 'languages': ['it'], "behaviorHints": {"notWebReady": True, "proxyHeaders": {"request": {"Origin": "https://babaktv.com", "Referer": "https://babaktv.com/"}}}})
                            else:
                                streams['streams'].append({'title':f"{Icon}Server {i} " + channel['title'] + " [IT]",'url': item, 'languages': ['it']})
                    if id in skystreaming and SKY == "1":
                        url,Host,Sky_Origin = await get_skystreaming(id,client)
                        i = i+1
                        if url:
                            streams['streams'].append({'title': f'{Icon}Server S {i}' + channel['title'] + " [IT]", 'url': url, 'languages': ['it'], "behaviorHints": {"notWebReady": True, "proxyHeaders": {"request": {"User-Agent": "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:127.0) Gecko/20100101 Firefox/127.0", "Accept": "*/*", "Accept-Language": "en-US,en;q=0.5", "Origin": Sky_Origin, "DNT": "1", "Sec-GPC": "1", "Connection": "keep-alive", "Referer": f"{Sky_Origin}/", "Sec-Fetch-Dest": "empty", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Site": "cross-site", "Pragma": "no-cache", "Cache-Control": "no-cache", "TE": "trailers","Host": Host}}}})
                    if id in webru_vary:
                        i = i+1
                        ...
                        if MFP== "1" and webru_url:
                            ...
                            streams['streams'].append({'title': f"{Icon}Proxied Server X-{i} " + channel['title'] + " [IT]", 'url': webru_url, 'languages': ['it']})
                        else:
                            if webru_url:
                                streams['streams'].append({'title': f'{Icon}Server X-{i}' + channel['title'] + " [IT]", 'url': webru_url, 'languages': ['it'], "behaviorHints": {"notWebReady": True, "proxyHeaders": {"request": {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3", "Accept": "*/*", "Accept-Language": "en-US,en;q=0.5", "Origin": Origin_webru_url, "DNT": "1", "Sec-GPC": "1", "Connection": "keep-alive", "Referer": Referer_webru_url, "Sec-Fetch-Dest": "empty", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Site": "cross-site", "Pragma": "no-cache", "Cache-Control": "no-cache", "TE": "trailers"}}}})
                    ...
                    if id in webru_dlhd:
                        if DLHD == "1":
                            ...
                            if MFP== "1":
                                ...
                                streams['streams'].append({'title': f"{Icon}Proxied Server D-{i} " + channel['title'] + " [IT]", 'url': webru_url_2, 'languages': ['it']})
                            else:
                                streams['streams'].append({'title': f'{Icon}Server D-{i}' + channel['title'] + " [IT]", 'url': webru_url_2, 'languages': ['it'], "behaviorHints": {"notWebReady": True, "proxyHeaders": {"request": {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3", "Accept": "*/*", "Accept-Language": "en-US,en;q=0.5", "Origin": Origin_webru_url_2, "DNT": "1", "Sec-GPC": "1", "Connection": "keep-alive", "Referer": Referer_webru_url_2, "Sec-Fetch-Dest": "empty", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Site": "cross-site", "Pragma": "no-cache", "Cache-Control": "no-cache", "TE": "trailers"}}}})
                ...
                if provider_maps['FILMPERTUTTI'] == "1" and FT == "1":
                    ...
                    if url_filmpertutti is not None and Host is not None:
                        if MFP == "1":
                            ...
                            streams['streams'].append({'name': f'{Name}', 'title': f'{Icon}Filmpertutti [IT]', 'url': url_filmpertutti, 'languages': ['it'], 'behaviorHints': {'bingeGroup': 'filmpertutti'}})
                        else:
                            streams['streams'].append({'name': f'{Name}', 'title': f'{Icon}Filmpertutti [IT]', 'url': url_filmpertutti, 'languages': ['it'], 'behaviorHints': {'proxyHeaders': {"request": {"User-Agent": "Mozilla/5.0 (Windows NT 10.10; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3"}}, 'notWebReady': True, 'bingeGroup': 'filmpertutti'}})
                    ...
                    if url_tantifilm != "{'Doodstream HD': None}" and url_tantifilm != None:
                        ...
                        if not isinstance(url_tantifilm, str):
                            for title, url in url_tantifilm.items():    
                                streams['streams'].append({'title': f'{Icon}Tantifilm {title} [IT]', 'url': url, 'languages': ['it'], 'behaviorHints': {'proxyHeaders': {"request": {"Referer": "https://d000d.com/"}}, 'notWebReady': True, 'bingeGroup': f'tantifilm{title}'}})
                        else:
                            streams['streams'].append({'title': f'{Icon}Tantifilm [IT]', 'url': url_tantifilm, 'languages': ['it'], 'behaviorHints': {'proxyHeaders': {"request": {"Referer": "https://d000d.com/"}}, 'notWebReady': True, 'bingeGroup': 'tantifilm'}})
            ...
            if provider_maps['GUARDASERIE'] == "1" and GS == "1":
                ...
                if url_guardaserie:
                    ...
                    streams['streams'].append({'name': f"{Name}",'title': f'{Icon}Guardaserie [IT]', 'url': url_guardaserie, 'languages': ['it'], 'behaviorHints': {'bingeGroup': 'guardaserie'}})
            if provider_maps['GUARDAHD'] == "1" and GHD == "1":
                url_guardahd = await guardahd(id,client)
                if url_guardahd:
                    print(f"GuardaHD Found Results for {id}")
                    streams['streams'].append({'name': f"{Name}",'title': f'{Icon}GuardaHD [IT]', 'url': url_guardahd, 'languages': ['it'], 'behaviorHints': {'bingeGroup': 'guardahd'}})
